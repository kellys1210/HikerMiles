...
router.get("/", (req, res) => {
  ...
  let select_patrons_query =
    "SELECT patron_id, (SELECT name FROM Patrons WHERE Patrons.patron_id = PatronParks.patron_id) AS patron_name FROM PatronParks"

  let select_parks_query =
    "SELECT park_id, (SELECT name FROM Parks WHERE Parks.park_id = PatronParks.park_id) AS park_name FROM PatronParks"
  ...
  db.pool.query(select_table_query, function (error, rows, fields) {
    ...
    db.pool.query(select_patrons_query, function(error, rows, fields) {
      ...
      db.pool.query(select_parks_query, function (error, rows, fields) {
        ...
        res.render("patron_parks", { patron_parks: data_table, patrons: data_patrons, parks: data_parks });
      });
    });
  });
});
